...
class Model:
    ...
    def update(self):
        ...
        for state_1, rules in self.rules.items():
            ...
            # find the total number of pairs in state_1, not counting new arrivals
            population = sum([1 for pair in self.pairs if pair.state == state_1 and not pair.changed])

            # multiply the rules for state_1 by the population to find out how many we need of each state_2
            # rules = {state: int(round(percent * population)) for (state, percent) in rules.items()}
            ## but if these dont add up to the population, we have a little rounding issue
            ## sum(rules.values()) - population == 0
            ## the issue here is that the algorithm below is always going to give short shrift to (--)
            ## if we always take the floor, rather than rounding up, nobody steals and it privileges identity transitions equally
            rules = {state: int(percent * population) for (state, percent) in rules.items()}
            ## ...but ironically, this makes Lazarsfeld's rules have a result that is even less close to magic distribution

            # for each state_2, check if each pair can be changed from state_1
            for state_2, num in rules.items():
                ...
                for n in range(num):                    
                    while p < len(self.pairs) and (self.pairs[p].changed or self.pairs[p].state != state_1):
                        p += 1
                    if p == len(self.pairs):
                        # we ran out of pairs before we could get enough of each
                        print('fail', state_1, state_2, num, n)   
                        break
